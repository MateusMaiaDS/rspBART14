...
rspBART <- function(x_train,
  ...
  interaction_matrix_list <- vector("list",length = NCOL(interaction_list))
  ...
  for(k in 1:data$n_tree){
    if(k <= NCOL(data$x_train)){
      forest[[k]] <- stump(data = data,j = k)
    } else {
      forest[[k]] <- stump(data = data, j = sample(1:NCOL(data$x_train),size = 1))
    }
  }
    ...
    for(t in 1:data$n_tree){
      ...
      if(verb == "grow"){
        ...
        if((stats::runif(n = 1)<0.5) & (interaction_term)){
          ...
          forest[[t]] <- prune_var(tree = forest[[t]],
                                           curr_part_res = partial_residuals,
                                           data = data)
        } else {
        ...
        if((stats::runif(n = 1)<0.5) & (interaction_term)){
          forest[[t]] <- change_var(tree = forest[[t]],
                                            curr_part_res = partial_residuals,
                                            data = data)
        } else {
